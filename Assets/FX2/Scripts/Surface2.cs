using System;
using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public struct PathPoint
{
    public Vector3 GeometricalPoint;
    public List<PathPoint> Nodes;

    public PathPoint(Vector3 geoPoint)
    {
        GeometricalPoint = geoPoint;
        Nodes = new List<PathPoint>();
    }
}


public class Surface2 : MonoBehaviour
{
    public static Surface2 instance;
    public Hexagon hexPrefab;
    public Transform WallPrefab;
    private Camera cam;
    public int height = 10;
    public int width = 10;
    public Vector3 ld;
    public Vector3 rd;
    public Vector3 lu;
    public Vector3 ru;
    public float w;
    public float h;

    private int[,] allHexXZ;//это для счета к какому хексу мышка наиближе
    public Hexagon[] allHex;//это хексы логики

    PathPoint[,] graph;
    public void Awake()
    {
        instance = this;
        Init();
    }


    public void Init()
    {
        cam = Camera.main;

        ld = cam.ScreenToWorldPoint(new Vector3(0, 0, 1f));

        rd = cam.ScreenToWorldPoint(new Vector3(Screen.width, 0, 1f));

        lu = cam.ScreenToWorldPoint(new Vector3(0, Screen.height, 1f));

        ru = cam.ScreenToWorldPoint(new Vector3(Screen.width, Screen.height, 1f));

        float radius = 2f;
        w = Mathf.Sin((60) * Mathf.Deg2Rad) * 2f * radius;//растояние по горизонтали между шестигольниками
        h = 2 * radius * 3f / 4f;//растояние по вертикали между шестигольниками

        height = Mathf.CeilToInt((lu.z - ld.z) / h) - 1;//находим количество шестиугольников в ширину и длину
        width = Mathf.CeilToInt((rd.x - ld.x) / w) - 1;

        allHexXZ = new int[width, height];
        allHex = new Hexagon[width * height];
        graph = new PathPoint[width, height];
        for (int z = 0; z < height; z++)
        {
            for (int x = 0; x < width; x++)
            {
                Vector3 hexPosition = new Vector3(w * (x + (z % 2f) / 2f), 0f, z * h);
                graph[x, z] = CreateHexagonGraph(hexPosition, radius);

                Hexagon hex = Instantiate(hexPrefab, new Vector3(w * (x + (z % 2f) / 2f), 0f, z * h), Quaternion.identity, transform);
                int id = z * width + x;
                allHexXZ[x, z] = id;//двумерный массив
                hex.id = id;
                allHex[id] = hex;

            }
        }

        MergeGraph(graph, width, height);
        Camera.main.transform.parent.position = new Vector3((width - 0.5f) * w / 2f, 0, (height - 1) * h / 2f);


        for (int i = 0; i < allHex.Length; i++)//записываем соседей
        {
            allHex[i].neighbors = new List<int>();
            for (int j = 0; j < allHex.Length; j++)
            {
                if (Vector3.Distance(allHex[i].transform.position, allHex[j].transform.position) < 4f)
                {
                    if (allHex[i] != allHex[j])
                    {
                        allHex[i].neighbors.Add(j);
                    }
                }
            }
        }
    }

    void Update()
    {
        for (int z = 0; z < height; z++)
        {
            for (int x = 0; x < width; x++)
            {
                graph[x, z].Nodes.ForEach(node => Debug.DrawLine(graph[x, z].GeometricalPoint, node.GeometricalPoint, Color.red));

            }
        }
    }

    public int PositionToId(Vector3 pos)
    {

        int z = (int)Mathf.Round(pos.z / h);
        int x = (int)Mathf.Round((pos.x - (z % 2) * 0.5f * w) / w);
        z = Mathf.Clamp(z, 0, allHexXZ.GetLength(1) - 1);
        x = Mathf.Clamp(x, 0, allHexXZ.GetLength(0) - 1);
        return allHexXZ[x, z];
    }


    public void AddWall(int id)
    {
        Hexagon hex = allHex[id];
        SetAllStateToNull(id);
        hex.isWall = true;
        hex.cost = -1;

        for (int i = 0; i < hex.transform.childCount; i++)//удаляет чайлды старой графики
        {
            GameObject.Destroy(hex.transform.GetChild(i));
        }


        Instantiate(WallPrefab, hex.transform.position, Quaternion.identity, hex.transform);

    }

    private void SetAllStateToNull(int id)
    {
        Hexagon hex = allHex[id];
        hex.isWall = false;
        hex.isDig = false;
        hex.isGround = false;
        hex.isBuild = false;
        hex.isSpawn = false;
    }

    private void MergeGraph(PathPoint[,] graph, int width, int height)
    {
        for (int z = 0; z <= height; z++)
        {
            for (int x = 0; x < width; x++)
            {
                if (x == 0 && z == 0)
                {
                    MergePathPoints(graph[x, z].Nodes[0], graph[x, z + 1].Nodes[3]);
                    MergePathPoints(graph[x, z].Nodes[1], graph[x + 1, z + 1].Nodes[5]);
                    MergePathPoints(graph[x, z].Nodes[2], graph[x + 1, z].Nodes[5]);
                }
                else if (x == 0)
                {
                    // MergePathPoints(graph[x, z].Nodes[0], graph[x, z + 1].Nodes[3]);
                    // MergePathPoints(graph[x, z].Nodes[1], graph[x + 1, z + 1].Nodes[4]);
                    // MergePathPoints(graph[x, z].Nodes[2], graph[x + 1, z].Nodes[5]);
                    // MergePathPoints(graph[x, z].Nodes[3], graph[x, z - 1].Nodes[0]);
                }
                else if (x == 0 && z % 2 == 0)
                {
                    // MergePathPoints(graph[x, z].Nodes[0], graph[x, z + 1].Nodes[3]);
                    // MergePathPoints(graph[x, z].Nodes[1], graph[x + 1, z + 1].Nodes[4]);
                    // MergePathPoints(graph[x, z].Nodes[2], graph[x + 1, z].Nodes[5]);
                    // MergePathPoints(graph[x, z].Nodes[3], graph[x, z - 1].Nodes[0]);
                }

                else if (x == 0 && z == height - 1)
                {
                    // MergePathPoints(graph[x, z].Nodes[2], graph[x + 1, z].Nodes[5]);
                    // MergePathPoints(graph[x, z].Nodes[3], graph[x, z - 1].Nodes[0]);
                }
                else if (z == height - 1 && x % 2 != 0)
                {
                    // MergePathPoints(graph[x, z].Nodes[1], graph[x + 1, z].Nodes[4]);
                    // MergePathPoints(graph[x, z].Nodes[2], graph[x + 1, z - 1].Nodes[5]);
                    // MergePathPoints(graph[x, z].Nodes[3], graph[x, z - 1].Nodes[0]);
                    // MergePathPoints(graph[x, z].Nodes[4], graph[x - 1, z - 1].Nodes[1]);
                    // MergePathPoints(graph[x, z].Nodes[5], graph[x - 1, z].Nodes[2]);
                }
                else if (z == height - 1 && x % 2 == 0)
                {
                    // MergePathPoints(graph[x, z].Nodes[2], graph[x + 1, z].Nodes[5]);
                    // MergePathPoints(graph[x, z].Nodes[3], graph[x, z - 1].Nodes[0]);
                    // MergePathPoints(graph[x, z].Nodes[4], graph[x - 1, z].Nodes[1]);
                }
                else if (x == width - 1 && z == height - 1)
                {
                    // MergePathPoints(graph[x, z].Nodes[3], graph[x, z - 1].Nodes[0]);
                    // MergePathPoints(graph[x, z].Nodes[4], graph[x - 1, z - 1].Nodes[1]);
                    // MergePathPoints(graph[x, z].Nodes[5], graph[x - 1, z].Nodes[2]);
                }
                else if (x == width - 1)
                {
                    // MergePathPoints(graph[x, z].Nodes[0], graph[x, z + 1].Nodes[3]);
                    // MergePathPoints(graph[x, z].Nodes[3], graph[x, z - 1].Nodes[0]);
                    // MergePathPoints(graph[x, z].Nodes[4], graph[x - 1, z - 1].Nodes[1]);
                    // MergePathPoints(graph[x, z].Nodes[5], graph[x - 1, z].Nodes[2]);
                }

                else if (x == width - 1 && z == 0)
                {
                    // MergePathPoints(graph[x, z].Nodes[0], graph[x, z + 1].Nodes[3]);
                    // MergePathPoints(graph[x, z].Nodes[5], graph[x - 1, z].Nodes[2]);
                }
                else if (z == 0 && x % 2 == 0)
                {
                    // MergePathPoints(graph[x, z].Nodes[0], graph[x, z + 1].Nodes[3]);
                    // MergePathPoints(graph[x, z].Nodes[1], graph[x + 1, z + 1].Nodes[4]);
                    // MergePathPoints(graph[x, z].Nodes[2], graph[x + 1, z].Nodes[5]);
                    // MergePathPoints(graph[x, z].Nodes[4], graph[x - 1, z].Nodes[1]);
                    // MergePathPoints(graph[x, z].Nodes[5], graph[x - 1, z + 1].Nodes[2]);
                }
                else if (z == 0 && x % 2 != 0)
                {
                    // MergePathPoints(graph[x, z].Nodes[0], graph[x, z + 1].Nodes[3]);
                    // MergePathPoints(graph[x, z].Nodes[1], graph[x + 1, z].Nodes[4]);
                    // MergePathPoints(graph[x, z].Nodes[5], graph[x - 1, z].Nodes[5]);
                }
                else if (x % 2 != 0)
                {
                    // MergePathPoints(graph[x, z].Nodes[0], graph[x, z + 1].Nodes[3]);
                    // MergePathPoints(graph[x, z].Nodes[1], graph[x + 1, z].Nodes[4]);
                    // MergePathPoints(graph[x, z].Nodes[2], graph[x + 1, z - 1].Nodes[5]);
                    // MergePathPoints(graph[x, z].Nodes[3], graph[x, z - 1].Nodes[0]);
                    // MergePathPoints(graph[x, z].Nodes[4], graph[x - 1, z - 1].Nodes[1]);
                    // MergePathPoints(graph[x, z].Nodes[5], graph[x - 1, z].Nodes[2]);
                }
                else if (x % 2 != 0)
                {
                    // MergePathPoints(graph[x, z].Nodes[0], graph[x, z + 1].Nodes[3]);
                    // MergePathPoints(graph[x, z].Nodes[1], graph[x + 1, z + 1].Nodes[4]);
                    // MergePathPoints(graph[x, z].Nodes[2], graph[x + 1, z].Nodes[5]);
                    // MergePathPoints(graph[x, z].Nodes[3], graph[x, z - 1].Nodes[0]);
                    // MergePathPoints(graph[x, z].Nodes[4], graph[x - 1, z].Nodes[1]);
                    // MergePathPoints(graph[x, z].Nodes[5], graph[x - 1, z + 1].Nodes[2]);
                }
            }
        }
    }


    public void MergePathPoints(PathPoint p1, PathPoint p2)
    {
        PathPoint p3 = new PathPoint((p1.GeometricalPoint + p2.GeometricalPoint) / 2);
        p3.Nodes.AddRange(p1.Nodes);
        p3.Nodes.AddRange(p2.Nodes);
        p1.GeometricalPoint = p2.GeometricalPoint = p3.GeometricalPoint;
        p1.Nodes = p2.Nodes = p3.Nodes;
    }

    private PathPoint CreateHexagonGraph(Vector3 center, float r)
    {
        // r - distance to edge
        // R - distance to angle = a size of a side
        // float R = 2 / (float)Math.Sqrt(3) * r;

        var centerPathPoint = new PathPoint(center);
        List<PathPoint> pathPoints = new List<PathPoint>();
        for (int n = 0; n < 6; n++)
        {
            float angle = (float)(Math.PI * (n * 60) / 180.0);
            float x = r * (float)Math.Cos(angle);
            float z = r * (float)Math.Sin(angle);
            pathPoints.Add(new PathPoint(center + new Vector3(x, 0, z)));
        }
        for (int i = 0; i < pathPoints.Count; i++)
        {
            centerPathPoint.Nodes.Add(pathPoints[i]);
            pathPoints[i].Nodes.Add(pathPoints[(i + 1) % pathPoints.Count]);
        }
        return centerPathPoint;

    }
}